...
|---|---|---|---|
| 基线（早期日志） | 10450 | 0 | 8806 |
| 改进前 bug 复现 | 72904 | 13733 | 31945 |
| 改进 v1（中间版本） | 18098 | 3151 | 8183 |
| 改进 v2（最终版本） | 18040 | 2244 | 5132 |
...
**前进/后退 21.4%**，**平移 1.6%**，
**旋转 14.5%**，**复合 0.7%**，
**静止 61.9%**。整体工况随样本序号变化的全景与色带标注见图 1。
...
|---|---|---|---|---|
| 1 | 0.0348 | 0.349 | 10.69 | 104.68 |
| 2 | 0.0363 | 0.356 | 11.15 | 104.47 |
| 3 | 0.0414 | 0.349 | 18.37 | 108.41 |
...
bug 数据集中检测到 **13** 次此类事件，最大堆叠峰值
$S_{\text{peak,max}} = 0.350$ m/s，最大释放阶跃
$\Delta V_{\text{max}} = 0.350$ m/s，最大舵角残余
$\Delta A_{\text{max}} = 14.120^\circ$。
...
- 改进前：堆叠峰值达 $S_{\text{peak}}\approx$0.350 m/s，
  cmdV 释放瞬间产生约 0.350 m/s 的阶跃。
- 改进后：堆叠峰值降至 $S_{\text{peak}}\approx$0.237 m/s，
  释放瞬间的 cmdV 阶跃仅 0.102 m/s，且 cmdVx 在
...
|---|---|---|---|---|
| 事件数 | 13 | 2 | 4 | 69.2% |
| 堵舵期 cmdVx 堆叠最大值 (m/s) | 0.350 | 0.350 | 0.237 | 32.3% |
| 释放阶跃最大值 (m/s) | 0.350 | 0.350 | 0.102 | 70.9% |
| 释放阶跃平均值 (m/s) | 0.213 | 0.350 | 0.040 | 81.2% |
...
- **释放阶跃最大值** 由 0.350 m/s 下降到
  0.102 m/s，**下降 70.9%**。
- **释放阶跃平均值** 由 0.213 m/s 下降到
  0.040 m/s，**下降 81.2%**——
  这是物理"过冲感"减轻最直接的指标。
- 三轮轮速 RMSE 也均有不同程度下降。
